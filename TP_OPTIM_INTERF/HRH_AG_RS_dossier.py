...
class Ui_HRH_AG_RS_dossier(QWidget):


    def loadData_mod_csv(self, fileName):

        with open(fileName, "rb") as useHRH_AG_RS:

            for row_number, user in enumerate(useHRH_AG_RS):
    ...
    def execDir(self):
        ...
        pat = pathh
        files = listdir(pathh)
        open("result_HRH_AG_RS.csv", "w").close()
        print(1)
        print(pat)
        for f in files:
            l = pat + "/" + f
            ...
            time = stop - start
            c = len(sa.bins)
            resultatt = open("result_HRH_AG_RS.csv", mode="a+")

            resultatt.write(f'{instance},{n},{C},{c},{j},{time * 1000}\n')  # Ecrire les resultats dans un fichier csv
            resultatt.close()
        print(pathh)
        self.loadData_mod_csv('./result_HRH_AG_RS.csv')

    def setupUi(self, HRH_AG_RS_dossier):
        HRH_AG_RS_dossier.setObjectName("HRH_AG_RS_dossier")
        HRH_AG_RS_dossier.resize(1000, 600)
        self.centralwidget = QtWidgets.QWidget(HRH_AG_RS_dossier)
        ...
        HRH_AG_RS_dossier.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(HRH_AG_RS_dossier)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 488, 18))
        self.menubar.setObjectName("menubar")
        HRH_AG_RS_dossier.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(HRH_AG_RS_dossier)
        self.statusbar.setObjectName("statusbar")
        HRH_AG_RS_dossier.setStatusBar(self.statusbar)

        self.retranslateUi(HRH_AG_RS_dossier)
        QtCore.QMetaObject.connectSlotsByName(HRH_AG_RS_dossier)

        self.pushButton.clicked.connect(self.execDir)
        self.pushButton_2.clicked.connect(self.openFileNameDialog)

    def retranslateUi(self, HRH_AG_RS_dossier):
        _translate = QtCore.QCoreApplication.translate
        HRH_AG_RS_dossier.setWindowTitle(_translate("HRH_AG_RS_dossier", "MainWindow"))
        self.label.setText(_translate("HRH_AG_RS_dossier", "Selectionner le dossier de test"))
        self.pushButton_2.setText(_translate("HRH_AG_RS_dossier", "Selectionner"))
        self.pushButton.setText(_translate("HRH_AG_RS_dossier", "Tester"))
...
if __name__ == "__main__":
    ...
    HRH_AG_RS_dossier = QtWidgets.QMainWindow()
    ui = Ui_HRH_AG_RS_dossier()
    ui.setupUi(HRH_AG_RS_dossier)
    HRH_AG_RS_dossier.show()
    sys.exit(app.exec_())
