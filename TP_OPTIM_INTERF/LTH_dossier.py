...
class Ui_LTH_dossier(QWidget):


    def loadData_mod_csv(self, fileName):

        with open(fileName, "rb") as useLTH:

            for row_number, user in enumerate(useLTH):
    ...
    def execDir(self):
        ...
        for f in files:
            ...
            resultatt = open("result_LTH.csv", mode="a+")

            resultatt.write(f'{instance},{n},{C},{c},{j},{time * 1000}\n')  # Ecrire les resultats dans un fichier csv
            resultatt.close()
        print(pathh)
        self.loadData_mod_csv('./result_LTH.csv')

    def setupUi(self, LTH_dossier):
        LTH_dossier.setObjectName("LTH_dossier")
        LTH_dossier.resize(1000, 600)
        self.centralwidget = QtWidgets.QWidget(LTH_dossier)
        ...
        LTH_dossier.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(LTH_dossier)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 488, 18))
        self.menubar.setObjectName("menubar")
        LTH_dossier.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(LTH_dossier)
        self.statusbar.setObjectName("statusbar")
        LTH_dossier.setStatusBar(self.statusbar)

        self.retranslateUi(LTH_dossier)
        QtCore.QMetaObject.connectSlotsByName(LTH_dossier)

        self.pushButton.clicked.connect(self.execDir)
        self.pushButton_2.clicked.connect(self.openFileNameDialog)

    def retranslateUi(self, LTH_dossier):
        _translate = QtCore.QCoreApplication.translate
        LTH_dossier.setWindowTitle(_translate("LTH_dossier", "MainWindow"))
        self.label.setText(_translate("LTH_dossier", "Selectionner le dossier de test"))
        self.pushButton_2.setText(_translate("LTH_dossier", "Selectionner"))
        self.pushButton.setText(_translate("LTH_dossier", "Tester"))
...
if __name__ == "__main__":
    ...
    LTH_dossier = QtWidgets.QMainWindow()
    ui = Ui_LTH_dossier()
    ui.setupUi(LTH_dossier)
    LTH_dossier.show()
    sys.exit(app.exec_())
