...
class Ui_HRH_dossier(QWidget):


    def loadData_mod_csv(self, fileName):

        with open(fileName, "rb") as useHRH:

            for row_number, user in enumerate(useHRH):
    ...
    def execDir(self):
        ...
        for f in files:
            ...
            time = stop - start
            c = len(result2.bins)
            resultatt = open("result_HRH.csv", mode="a+")

            resultatt.write(f'{instance},{n},{C},{c},{j},{time * 1000}\n')  # Ecrire les resultats dans un fichier csv
            resultatt.close()
        print(pathh)
        self.loadData_mod_csv('./result_HRH.csv')

    def setupUi(self, HRH_dossier):
        HRH_dossier.setObjectName("HRH_dossier")
        HRH_dossier.resize(1000, 600)
        self.centralwidget = QtWidgets.QWidget(HRH_dossier)
        ...
        HRH_dossier.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(HRH_dossier)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 488, 18))
        self.menubar.setObjectName("menubar")
        HRH_dossier.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(HRH_dossier)
        self.statusbar.setObjectName("statusbar")
        HRH_dossier.setStatusBar(self.statusbar)

        self.retranslateUi(HRH_dossier)
        QtCore.QMetaObject.connectSlotsByName(HRH_dossier)

        self.pushButton.clicked.connect(self.execDir)
        self.pushButton_2.clicked.connect(self.openFileNameDialog)

    def retranslateUi(self, HRH_dossier):
        _translate = QtCore.QCoreApplication.translate
        HRH_dossier.setWindowTitle(_translate("HRH_dossier", "MainWindow"))
        self.label.setText(_translate("HRH_dossier", "Selectionner le dossier de test"))
        self.pushButton_2.setText(_translate("HRH_dossier", "Selectionner"))
        self.pushButton.setText(_translate("HRH_dossier", "Tester"))
...
if __name__ == "__main__":
    ...
    HRH_dossier = QtWidgets.QMainWindow()
    ui = Ui_HRH_dossier()
    ui.setupUi(HRH_dossier)
    HRH_dossier.show()
    sys.exit(app.exec_())
