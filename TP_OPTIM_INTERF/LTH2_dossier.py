...
class Ui_LTH2_dossier(QWidget):


    def loadData_mod_csv(self, fileName):

        with open(fileName, "rb") as useLTH2:

            for row_number, user in enumerate(useLTH2):
    ...
    def execDir(self):
        ...
        for f in files:
            ...
            resultatt = open("result_LTH2.csv", mode="a+")

            resultatt.write(f'{instance},{n},{C},{c},{j},{time * 1000}\n')  # Ecrire les resultats dans un fichier csv
            resultatt.close()
        print(pathh)
        self.loadData_mod_csv('./result_LTH2.csv')

    def setupUi(self, LTH2_dossier):
        LTH2_dossier.setObjectName("LTH2_dossier")
        LTH2_dossier.resize(1000, 600)
        self.centralwidget = QtWidgets.QWidget(LTH2_dossier)
        ...
        LTH2_dossier.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(LTH2_dossier)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 488, 18))
        self.menubar.setObjectName("menubar")
        LTH2_dossier.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(LTH2_dossier)
        self.statusbar.setObjectName("statusbar")
        LTH2_dossier.setStatusBar(self.statusbar)

        self.retranslateUi(LTH2_dossier)
        QtCore.QMetaObject.connectSlotsByName(LTH2_dossier)

        self.pushButton.clicked.connect(self.execDir)
        self.pushButton_2.clicked.connect(self.openFileNameDialog)

    def retranslateUi(self, LTH2_dossier):
        _translate = QtCore.QCoreApplication.translate
        LTH2_dossier.setWindowTitle(_translate("LTH2_dossier", "MainWindow"))
        self.label.setText(_translate("LTH2_dossier", "Selectionner le dossier de test"))
        self.pushButton_2.setText(_translate("LTH2_dossier", "Selectionner"))
        self.pushButton.setText(_translate("LTH2_dossier", "Tester"))
...
if __name__ == "__main__":
    ...
    LTH2_dossier = QtWidgets.QMainWindow()
    ui = Ui_LTH2_dossier()
    ui.setupUi(LTH2_dossier)
    LTH2_dossier.show()
    sys.exit(app.exec_())
