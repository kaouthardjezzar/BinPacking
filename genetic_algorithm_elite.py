...
class GeneticAlgorithm:
    ...
    def run(self):
        ...
        while num_no_change < self.MAX_NO_CHANGE and current_iteration < self.MAX_GENERATIONS:
            new_generation = []
            elite_gen = self.elite_selection()
            while len(new_generation) < self.POPULATION_SIZE:
                # Select parents     
                parent1 = secrets.choice(elite_gen)
                parent2 = secrets.choice(elite_gen)
    ...
    def elite_selection(self):
        """
        Select elite parents from the current population by applying elitist selection
        :return: Population_size / 2 first individuals with the highest fitness
        """
        self.population = sorted(self.population,key= lambda indiv : indiv.fitness,reverse=True)
        subpop = []
        for i in range(ceil(len(self.population)/2)):
            subpop.append(self.population[i])
        return subpop
